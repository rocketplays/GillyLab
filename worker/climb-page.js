...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* The archetype is a name, not a number — the HUD's 1.1rem is sized for "10-0"
     and "Chain grappler" at that size wraps the row on a phone. */
...
// falls through to "X-Y" (see below), which reads fine: "Wrestler-Grinder".
...
// striker" instead of "Pressure fighter", wrestling+grappling read "BJJ
// specialist" instead of "Chain grappler". A lookup table whose keys are built by
...
// and hand-written order stops being load-bearing.
const ARCH_PAIR = Object.fromEntries(Object.entries({
  'power+wrestling':     'Wrestle-boxer',
  ...
  'cardio+grappling':    'Submission grinder',
  'cardio+takedef':      'Stick-and-move',
  'wrestling+grappling': 'Chain grappler',
  'wrestling+takedef':   'Mat general',
  'grappling+takedef':   'Guard player',
...
function fight(o){
  ...
  const finW = won ? (0.25 + finBias * 0.4)          // you finish: 0.25 -> 0.65
                   : (0.25 + threat  * 0.4) * chinGuard;  // he finishes: 0.35 -> 0.61, less your chin
  const fin = Math.random() < finW && (won || kd + sb > 0);
  // Weights for WHICH finish. Yours: power vs grappling. His: knockdowns vs subs.
  const koW  = won ? (G.attrs.power||1)          : kd;
  const subW = won ? (G.attrs.grappling||1) * 0.9 : sb;
...
function hud(){
  ...
  d.innerHTML =
    ...
    // on wrestling and a Knockout artist becomes a Wrestle-boxer, and you should
...
function resultBox(){
  ...
  n.textContent = won
    ? '+'+(o.reward+(fin?1:0))+' upgrade points.'
