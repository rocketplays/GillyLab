...
function applyResult(o, R){
  ...
  if (won){
    ...
    if (G.champ) { G.champ = false; G.wasChamp = true; }
...
function offerBox(){
  ...
  for(const o of list){
    ...
    b.innerHTML='<div class="opphd">'+avatarHTML(o.f)+'<div class="oppwho">'+
      ...
      '<div class="rw">'+(o.defense ? ('Defend the belt · +'+o.reward+' pt') : ('+'+o.reward+' upgrade pts if you win'))+'</div>';
...
function cornerLine(iq, s, rd, coach){
  ...
  switch (Math.max(1, Math.min(10, iq|0))) {
    ...
    case 4:  return C("Keep it "+s+". "+cap(rd)+".");
    case 5:  return C("Keep it "+s+", "+rd+", and fight your fight.");
    case 6:  return C(ot+". Keep it "+s+", "+rd+".");
    case 7:  return C(ot+". Keep it "+s+", "+rd+" — that's the game plan.");
    case 8:  return C("Keep it "+s+", "+rd+" — do that and it's your night.");
    case 9:  return C("Keep it "+s+", "+rd+". And watch it: "+tr+".");
    default: return C(ot+". Keep it "+s+", "+rd+", and whatever happens, "+tr+". Clear as day.");
  }
}
...
function planBox(){
  ...
  // THE CORNER'S ACTUAL PICKS — highest-edge WHERE you can run, highest-edge READ.
  // At high Fight IQ (>=8) they're marked right on the buttons so the optimal plan is
  // unmistakable — that's what your Fight IQ points bought, paid for in the combat
  // levels you skipped. Below that you read the pros and cons and back your own gut.
  ...
  const showPick = effIQ >= 8;

  const group = (labelText, arr, kind, pickId)=>{
    ...
    arr.forEach(op=>{
      const isPick = showPick && op.id===pickId;
  ...
  wrap.appendChild(group('Where do you want the fight?', plan.where, 'where', bestW.id));
  wrap.appendChild(group('Key read', plan.read, 'read', bestR.id));
...
function runSummary(){
  ...
  return {
    ...
    // didn't.
    verdict: G.champ ? (legacyTier() && legacyTier().goat ? 'THE GOAT.' : 'CHAMPION.')
