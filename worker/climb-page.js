...
function fight(o){
  ...
  // NORMALISED BY (ATTR_MAX*2), THE SAME DIVISOR AS BEFORE, so the two primary
  // avenues are untouched: a 10-power/1-grappling slugger still reads 0.55 and
  // still finishes 47% of his wins, exactly as he did. Wrestling and pace only ever
  // ADD. Clamped at 1 so the ceiling — a 65% finish rate — doesn't move either;
  // this widens WHO can finish, it does not make finishes more common overall.
  const finBias = Math.min(1, (
      (G.attrs.power||0)     * FIN_W.power +
