...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* ── SIGNATURE PICKER ─────────────────────────────────────────────────────── */
  .siggrid{display:grid;grid-template-columns:1fr 1fr;gap:.5rem}
  .sigcard{background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:.55rem .65rem;text-align:left;cursor:pointer;font-family:inherit;color:var(--text)}
  .sigcard:hover{border-color:var(--muted)}
  .sigcard.on{border-color:var(--accent);background:rgba(0,230,104,.07)}
  .sgh{font-weight:700;font-size:.82rem;margin-bottom:.25rem;color:var(--gold)}
  .sigcard.on .sgh{color:var(--accent)}
  .sge{font-size:.72rem;line-height:1.4;color:var(--text)}
  .sgf{font-size:.68rem;line-height:1.35;color:var(--muted);margin-top:.3rem}
  .sgf b{color:var(--accent2)}
  .sigv{font-size:.78rem}
  @media(max-width:560px){ .siggrid{grid-template-columns:1fr} }
...
const SIGS = [
  { id:'killer',  name:'Killer Instinct', icon:'ti-bolt',
  ...
  { id:'chin',    name:'Iron Chin', icon:'ti-shield-half',
  ...
  { id:'scram',   name:'Scrambler', icon:'ti-arrows-shuffle',
  ...
  { id:'dog',     name:'Dog in Him', icon:'ti-flame',
  ...
  { id:'general', name:'Ring General', icon:'ti-eye',
    line:"You read a fight cold. Your corner always sees it clearly and a sharp plan cuts deeper.",
    flaw:"You out-point people — you rarely take anyone out." },
  { id:'power',   name:'Walk-Off Power', icon:'ti-hammer',
...
function creator(){
  ...
  const sigCard=(s)=>{
    ...
    c.className='sigcard'+(((s?s.id:null)===(G.sig||null))?' on':'');
    if(s){
      c.innerHTML='<div class="sgh"></div><div class="sge"></div><div class="sgf"><b>Wrinkle </b><span></span></div>';
      c.querySelector('.sgh').textContent=s.name;
      c.querySelector('.sge').textContent=s.line;
      c.querySelector('.sgf span').textContent=s.flaw;
      c.onclick=()=>{ G.sig=s.id; render(); };
    } else {
      c.innerHTML='<div class="sgh"></div><div class="sge"></div>';
      c.querySelector('.sgh').textContent='No signature';
      c.querySelector('.sge').textContent='A pure all-rounder — no edge, and no flaw.';
      c.onclick=()=>{ G.sig=null; render(); };
    }
    return c;
  };
  sgg.appendChild(sigCard(null));
  SIGS.forEach(s=>sgg.appendChild(sigCard(s)));
  sgw.appendChild(sgg); p.appendChild(sgw);
...
function planBox(){
  ...
  corner.textContent=cornerLine(G.sig==='general'?ATTR_MAX:(G.attrs.fightiq||ATTR_MIN), bestW.short||bestW.label.toLowerCase(), plan.coach);
