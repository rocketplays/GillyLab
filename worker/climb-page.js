...
function render(){
  ...
  if (G.retired){ endFight(); app.appendChild(endBox('RETIRED on top — '+(G.defenses||0)+' title defense'+((G.defenses||0)===1?'':'s')+'.')); return; }
...
function offerBox(){
  ...
  if (G.champ){
    ...
    ch.innerHTML='<b>You hold the belt.</b> '+(G.defenses?('Defended '+G.defenses+' time'+(G.defenses===1?'':'s')+'. '):'')+'Take a challenger, or retire on top.';
    p.appendChild(ch);
  }
