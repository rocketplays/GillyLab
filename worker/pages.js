...
(function(){
  ...
  var slides=[
    ...
    // FREE ORDER IS DELIBERATE: lead with the detailed fighter statistics (the core of the
    // product), then the two things you can DO without an account — Pick'em, then The Climb —
    // and finish with what you can read (rankings & roster).
...
<script src="/gl-sheet.js?v=b22a40e5" defer></script>
<style>
...
export function pagesConsensusOdds(oddsData, nameA, nameB) {
  ...
  // -- ports index.html's own namesLikelyMatch()/levenshteinDist() (used the
  // same way there, as findFightOdds()'s fallback after its own lastNameOf()
  // match fails). Needed for cases a generic suffix list can't safely cover:
  // "Ilimbek Akylbek Uulu" (our DB spelling) vs odds.json's "Ilimbek Akylbek"
  // (drops the Kyrgyz patronymic "Uulu") never share a last WORD, but they do
  // share real name tokens. Adding "uulu" to SUFFIXES above would be the
  // wrong fix -- unlike Jr./Sr., it's not reliably a suffix to strip, and
  // doing so risks the same false-collision failure mode the Jr./Jr. bug
  // just was for two unrelated fighters who both happen to carry it.
  ...
  const nameTokens = (name) => String(name || "").toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/[^a-z\s]/g, "").trim().split(/\s+/).filter(Boolean);
  const namesLikelyMatch = (nameX, nameY) => {
...
<section class="faq">
    <div class="faq-list">
      <details class="faq-item"><summary>What's included?<span class="faq-chev">›</span></summary><div class="faq-body"><p>Every UFC fighter and bout: full career analytics, the fight simulator, per-fight box scores, career accolades, matchup analysis — each fighter’s style, pace and path to victory — the matchup deep dive, live odds and props, the bet &amp; CLV tracker, line-movement history, the parlay builder, closing-line history, tape links for every fight, division rankings, and weekly roster updates.</p></div></details>
