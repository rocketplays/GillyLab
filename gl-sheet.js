...
const GL_SHEET = (function () {
  ...
  function pkBoutCard(ctx, p, num, x, y, w, imgWin, imgLose, graded, s) {
    const R = 42 * s;
    const headY = y + 20 * s;
    ctx.textAlign = 'left'; ctx.font = '800 ' + (19 * s).toFixed(1) + 'px ' + COND; ctx.fillStyle = MUT;
    ...
    if (!graded) {
      ...
      ctx.fillText(clip(ctx, p.confidence + ' pick · ' + pkMethodLabel(p), w - 40 * s), x + w / 2, lineY);
      ctx.textAlign = 'left';
    } else if (p.voided) {
  ...
  async function drawPickem(data) {
    ...
    // fight-card renderer's own hasFlag/isMainCard/isMain split): a graded
    // (completed) card stays one flat block, matching "past events aren't
    // split into main card / prelims" there. An ungraded picks card gets the
    // main event pinned to its own full-width row, the rest of the main card
    // below it, then a PRELIMS divider and the rest — so the sheet reads in
    // the same order the event itself does, not just the order picks were
    // made in.
    const gap = 28, divH = 56;
    let mainEvent = null, mainCardRest = picks, prelims = [];
    if (!graded) {
      const hasFlag = picks.some(p => p.isMainCard);
      const mainCardAll = hasFlag ? picks.filter(p => p.isMainCard || p.isMain) : picks.filter(p => p.isMain);
      prelims = hasFlag ? picks.filter(p => !p.isMainCard && !p.isMain) : picks.filter(p => !p.isMain);
      mainEvent = mainCardAll.find(p => p.isMain) || null;
      mainCardRest = mainCardAll.filter(p => p !== mainEvent);
    }
    ...
    let cy = listTop, num = 1;
    if (mainEvent) {
      const imgs = imgFor.get(mainEvent);
      pkBoutCard(ctx, mainEvent, num++, 64, cy, W - 128, imgs[0], imgs[1], graded, 1);
      cy += ROW_H;
    }
    if (mainCardGrid) {
      mainCardRest.forEach((p, i) => {
        ...
        pkBoutCard(ctx, p, num++, bx, by, mainCardGrid.colW, imgs[0], imgs[1], graded, mainCardGrid.s);
      });
      cy += mainCardGrid.h;
    }
    if (prelimsGrid) {
      ...
      prelims.forEach((p, i) => {
        ...
        const imgs = imgFor.get(p);
        pkBoutCard(ctx, p, num++, bx, by, prelimsGrid.colW, imgs[0], imgs[1], graded, prelimsGrid.s);
      });
      cy += prelimsGrid.h;
    }
