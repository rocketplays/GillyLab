...
const GL_SHEET = (function () {
  ...
  async function drawPickem(data) {
    ...
    // Two bouts per row once there's more than a couple — one column per bout
    // scrolls to 2500-3000px for a full 11-14 fight card, nothing like the
    // reference poster; two columns roughly halves that, closer to a square,
    // shareable image while still keeping every fighter's photo legible. `s`
    // (see pkBoutCard) shrinks every size in a bout card to fit its column.
    const cols = n <= 2 ? 1 : 2;
    ...
    const CH = Math.round(listTop + rows * rowH + footerBlockH);
    ...
    picks.forEach((p, i) => {
      ...
      const bx = 64 + col * (colW + gap), by = listTop + row * rowH;
      pkBoutCard(ctx, p, i + 1, bx, by, colW, imgPairs[i][0], imgPairs[i][1], graded, s);
    });
