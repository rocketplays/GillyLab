...
const GL_SHEET = (function () {
  ...
  async function drawMatchup(nameA, nameB, ctxInfo, fmt) {
    ...
    if (story) {
      ...
      // Style bar labels/dots are always green for both fighters, not keyed to
      // champion status like the tape/h2h rows above.
      styleBar(ctx, y + 76, ins.leanA, ins.leanB, ACC, ACC, a.name, b.name);
      ...
      // Path-to-victory box/name are always green for both fighters too.
      if (ins.pathA) y = pathBlock(ctx, a.name, ins.pathA, ACC, y, 4);
      if (ins.pathB) y = pathBlock(ctx, b.name, ins.pathB, ACC, y, 4);
    }
  ...
  async function drawSim(nameA, nameB, result, rounds) {
    ...
    // Both fighters share the same accent green now — name, bars, and
    // percentages alike — rather than only the model's favourite getting colour.
    const colA = ACC, colB = ACC;
  ...
  // actual result once it's in.
  //
  // Drawn into a (x, y, w) cell rather than always the full card width — a
  // full 11-14 fight card in one column runs to 2500-3000px tall, nothing
  // like the reference poster's proportions, so drawPickem tiles bouts two
  // to a row once there's more than a couple. `s` is that cell's width as a
  // fraction of the single-column width every constant below was tuned
  // against (952px) — every size in here scales by it, so a two-column cell
  // is a faithful shrink of the one-column bout, not a separately-tuned
  // layout. Row height scales the same way (see ROW_H below), so every
  ...
  function pkBoutCard(ctx, p, x, y, w, imgWin, imgLose, graded, s) {
    ...
    const R = 53 * s;
    // headY/avY gap tightened now that most rows draw no text up here at all
    // (only MAIN EVENT and, on a graded card, the points badge still use this
    // line) — the old offsets were sized for a header that used to always
    // carry the division/rounds tag.
    const headY = y + 18 * s;
  ...
  // Base row height at s=1 (a single full-width column, 952px). A grid cell
  // at scale s uses ROW_H*s — see pkBoutCard's note on why that keeps every
  ...
  async function drawPickem(data) {
    ...
    // Group like the event page already does elsewhere in this file (see the
    // fight-card renderer's own hasFlag/isMainCard/isMain split): main event
    // pinned to its own full-width row, the rest of the main card below it,
    // then a PRELIMS divider and the rest — so the sheet reads in the same
    // order the event itself does. Applied to both the picks card and the
    // graded results card, so "Share results" isn't structured differently
    // from "Share picks" — just with results shown instead of picks.
    const gap = 28, divH = 56;
    const hasFlag = picks.some(p => p.isMainCard);
    const hasMain = picks.some(p => p.isMain);
    // sharePickemHistory()'s payload (the "My history" per-event share, a
    // different caller than sharePayload()) never sets isMain or isMainCard
    // at all — there's no card-segment data to split on there, so without
    // this guard every one of its bouts would fall into "!isMain" and the
    // whole card would render as one big, wrongly-labeled PRELIMS section.
    // Only split when there's an actual main event or isMainCard flag to
    // split on; otherwise render flat, same as before this feature existed.
    const canSplit = hasFlag || hasMain;
    const mainCardAll = !canSplit ? picks : (hasFlag ? picks.filter(p => p.isMainCard || p.isMain) : picks.filter(p => p.isMain));
    const prelims = !canSplit ? [] : (hasFlag ? picks.filter(p => !p.isMainCard && !p.isMain) : picks.filter(p => !p.isMain));
    const mainEvent = mainCardAll.find(p => p.isMain) || null;
    const mainCardRest = mainCardAll.filter(p => p !== mainEvent);
    // Two bouts per row within a section once it has more than a couple —
    // one column per bout scrolls to thousands of px for a full 11-14 fight
    // card, nothing like the reference poster; two columns keeps it closer
    // to square while still keeping every fighter's photo legible. `s` (see
    // pkBoutCard) shrinks every size in a bout card to fit its column.
    function gridDims(arr) {
      // <=1 (not <=2) — a section with exactly 2 bouts should still tile
      // side by side, not stack. The <=2 threshold this replaced made sense
      // when it ran once over the whole card (a 2-bout card genuinely reads
      // better as one full-width column), but now that it runs per section
      // (main card, prelims) a 2-bout section is common and stacking it
      // vertically is exactly the tall, un-square shape splitting into rows
      // was meant to avoid.
    ...
    // them the same way meta() does. Keyed by pick object since the draw order
    // below (main event, then main card, then prelims) isn't picks' own order.
    ...
    const imgFor = new Map(picks.map((p, i) => [p, imgPairs[i]]));

    let cy = listTop;
    if (mainEvent) {
      const imgs = imgFor.get(mainEvent);
      pkBoutCard(ctx, mainEvent, 64, cy, W - 128, imgs[0], imgs[1], graded, 1);
      cy += ROW_H;
    }
    if (mainCardGrid) {
  ...
  // ── FULL EVENT CARD (no picks — every scheduled bout, photos + names only) ──
  // The "Share this card" button on the home page and the upcoming-events
  // carousel: a poster of the whole card, not any one user's picks. Same
  // main-card/prelims data (isMain/isMainCard) as Pick'em above, but there's
  // no confidence ring, badge, or method line to draw — every fighter here
  // is unpicked, so the only two things worth showing are the photo and the
  // name, per feedback, which is also why the bouts can run bigger than a
  // Pick'em row: nothing else is competing for the vertical space.
  function evGroups(fights) {
    const hasFlag = fights.some(f => f.isMainCard);
    const mainCardAll = hasFlag ? fights.filter(f => f.isMainCard || f.isMain) : fights.filter(f => f.isMain);
    const prelims = hasFlag ? fights.filter(f => !f.isMainCard && !f.isMain) : fights.filter(f => !f.isMain);
    // "Co-main" isn't a flag anywhere in this data — it's just the next
    // main-card bout after the main event in billed order, same as how a
    // real card is read top to bottom.
    const mainEvent = mainCardAll.find(f => f.isMain) || mainCardAll[0] || null;
    const coMain = mainCardAll.find(f => f !== mainEvent) || null;
    const restMainCard = mainCardAll.filter(f => f !== mainEvent && f !== coMain);
    return { mainEvent, coMain, restMainCard, prelims };
  }
  // Centres a photo pair within a `w`-wide slot, deriving both photo
  // x-positions from the radius itself (2R + a gap scaled the same way)
  // instead of a fixed fraction of `w`. A fraction like 0.27/0.73 only
  // avoids the photos overlapping for the exact R it was tuned against —
  // this was the actual bug behind the hero row's main-event and co-main
  // photos overlapping at their inner edges: 100px-radius photos in a
  // pairW=366 slot at 0.27/0.73 need to sit only 168px apart, 32px closer
  // than two 100px-radius circles can get without touching.
  function evPairX(x0, w, R, vsGap) {
    const span = 4 * R + vsGap;               // both circles + the VS gap
  ...
  // last name is short enough to fit on one line at a much bigger size in
  // the large majority of cases (see evDrawName for the rest).
  //
  // A generational suffix ("Jr.", "III", ...) is NOT the surname — it's
  // the last whitespace-split token, though, so a naive "take the last
  // word" reads "Michael Aswell Jr." as "Jr." and drops the actual name.
  // Strip any of these off the end before taking what's left as the name.
  const EV_NAME_SUFFIXES = new Set(['jr', 'sr', 'ii', 'iii', 'iv', 'v']);
  function evLastName(full) {
    const parts = String(full || '').trim().split(/\s+/).filter(Boolean);
    while (parts.length > 1 && EV_NAME_SUFFIXES.has(parts[parts.length - 1].toLowerCase().replace(/\.$/, ''))) {
      parts.pop();
    }
  ...
  // straight into the canvas instead of relying on page CSS, so the share
  // image reads as GillyLab's even divorced from the site around it.
  ...
  // The "gillylab" wordmark, centred on `cx` — same two-tone treatment as
  // brand() (GILLY in TXT, LAB in ACC, same condensed font) but centred
  // instead of left-aligned, for the hero row's centre column.
  function brandCentered(ctx, cx, y, size) {
    ctx.textAlign = 'left'; ctx.font = '800 ' + size + 'px ' + COND;
    const gW = ctx.measureText('GILLY').width, lW = ctx.measureText('LAB').width;
    const x0 = cx - (gW + lW) / 2;
    ctx.fillStyle = TXT; ctx.fillText('GILLY', x0, y);
    ctx.fillStyle = ACC; ctx.fillText('LAB', x0 + gW, y);
    ctx.textAlign = 'center';
  }
  // A section divider bar spanning the card's content width — same visual
  // language as .fight-dd-bar, the "Matchup Analytics Deep Dive" button on
  ...
  function evGridDims(arr) {
    ...
    // columns, wrapping into extra rows, which is what keeps a deep prelims
    // section from scrolling forever the way an un-gridded list would.
    const cols = arr.length <= 1 ? 1 : (arr.length === 2 ? 2 : 3);
    const rows = Math.ceil(arr.length / cols);
    const evGap = 24;
    const colW = (W - 128 - evGap * (cols - 1)) / cols;
    // 300 (not 952, and not 620 like Pick'em's grid) — tuned so a 3-column
    // row, the densest case here, still renders at s=1: no pick/method line
    // is competing for width in this card, so there's no reason to shrink
    // fonts and photos below their base size just because there are three
  ...
  async function drawEventCard(data) {
    ...
    // Load every fighter's photo up front, across all three groups.
    const allFights = (mainEvent ? [mainEvent] : []).concat(coMain ? [coMain] : []).concat(restMainCard, prelims);
    ...
    if (mainEvent) {
      ...
      // for, mirroring the reference poster's centre roundel + title.
      const centerW = 220;
      const pairW = (W - 128 - centerW) / 2;
      const leftX = 64, rightX = 64 + pairW + centerW, centerX = 64 + pairW + centerW / 2;
      const avY = cy + heroR;
      ...
      // Centre mark: the GL fist, then the "gillylab" wordmark, then the
      // event name (no date/venue — that's the only thing the reference
      // poster puts in the middle, and this card only needs "UFC 331").
      // Stacked with real gaps between each, not packed tight under the
  ...
  async function drawBetCard(data) {
    ...
    // Rows are variable height: a parlay expands to list its legs vertically below the
    // header, on tight rows so it doesn't run long.
    const PLEG_ROW = 40, PHEAD_H = 84;
    const cardH = bets.map(b => (b.legs && b.legs.length) ? (PHEAD_H + b.legs.length * PLEG_ROW + 14) : (rowH - 12));
  ...
  return {
    ...
    // recipient cannot open; the image is the whole gift. Climb (below) is the one
    // free sheet that still keeps its share line — that link is how a Climb run
    // in a group chat brings someone in. Pick'em dropped its share line per
    // feedback; Save photo covers it now.
    ...
    // No shareText (null) — per feedback, Save photo is the only action
    // needed here; drops the Share button the same way matchup/sim/eventCard
    // already do (see open()'s own switch on shareText).
