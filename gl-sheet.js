...
const GL_SHEET = (function () {
  ...
  async function drawEventCard(data) {
    ...
    const gap = 24, divH = 56;
    const heroTop = 56, heroR = 72, heroVsGap = 56;
    // Hero row height: photo diameter + gap to the name block + 2 reserved
    // name lines (see evDrawName) + bottom padding. Fixed, like every other
    // row height in this module — the fonts inside it are constants.
    ...
    const bodyH = heroH + (mainCardGrid ? mainCardGrid.h : 0) + (prelimsGrid ? divH + prelimsGrid.h : 0);
    const footerH = 90;
    const CH = Math.round(bodyH + footerH);
    ...
    if (mainEvent) {
      ...
      // Centre mark: the GL fist, then the event name, then its date —
      // vertically centred on the same line the photos sit on.
      ctx.textAlign = 'center';
      if (logo && logo.width) {
        const lh = 56, lw = logo.width * (lh / logo.height);
        ctx.drawImage(logo, centerX - lw / 2, avY - heroR - 4, lw, lh);
      }
      ctx.font = '800 26px ' + COND; ctx.fillStyle = TXT;
      const nameLines = wrap(ctx, data.name || 'UFC', centerW - 16).slice(0, 2);
      let ny = avY - heroR + 78;
      nameLines.forEach((line) => { ctx.fillText(line, centerX, ny); ny += 30; });
      if (data.details) {
        ctx.font = '400 18px ' + SANS; ctx.fillStyle = MUT;
        ctx.fillText(clip(ctx, data.details, centerW - 8), centerX, ny + 4);
      }
      ctx.textAlign = 'left';
