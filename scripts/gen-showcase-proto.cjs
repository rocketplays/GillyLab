...
${footCSS}
  ...
  /* TWO COLUMNS ON DESKTOP. minmax(460px) means the grid fits two cards across inside
     the 1140px .fx-wrap and drops to one below ~940px — so a wide screen shows the settled
     2-up block, a narrow desktop shows a single readable column, and the phone (handled in
     the max-width:720px block below) turns each group into a swipe row instead.
     BELOW 720px IS NOT THIS GRID: two columns on a 390px phone gives cards 174px wide, and
     these payloads are authored for ~580px — they'd collapse into rubble, so the phone gets
     the swipe row, not a squeeze. The stacked Free / Premium blocks and the sticky pill that
     jumps between them are the landing's settled layout; the pill is desktop-only (its
     band-free / band-prem targets only exist when the two flat tiers render). */
  .fx-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(460px,1fr));gap:20px;grid-auto-flow:dense}

  /* The Free/Premium jump pill. Sticks below the hero once you scroll past it, and each chip
     scrolls to its tier band; the scroll-spy keeps the lit chip matching what you're looking
     at. Hidden under 720px, where the four group headings do the navigating instead. */
  ...
  .fx-fb.on{background:var(--accent);color:#0a0a0b}
  .fx-fb .n{opacity:.6;font-size:11px;margin-left:5px}
  @media (max-width:720px){ .fx-filter{display:none} }
  ...
  /* THE HUB CARD SPANS THE WHOLE ROW, so it gets ~1096px and renders at ~1:1 instead of
     the half-size thumbnail it was at 520px. It is also the headline premium feature, so
     a full-width card is the right hierarchy anyway — this is one of those rare cases
     where the layout fix and the design answer are the same thing. */
  .fx-card.wide{grid-column:1 / -1}
  .fx-card.wide .fx-scaler{width:1040px;transform-origin:top left;padding:14px 16px}
  .fx-card.wide .fx-frame{height:360px}
...
<div class="fx-wrap">
  <div class="fx-top">
    ...
    <!-- Free / Premium jump pill. Centred here in the hero because .fx-top is text-align
         centre; the script fills it and wires each chip to scroll to its tier band. Desktop
         only (CSS hides it under 720px); empty on /subscribe because that slice has no hero. -->
...
${slideJS}
  ...
  // FREE / PREMIUM ARE JUMP LINKS, NOT FILTERS. Both blocks always render; the two chips
  // scroll to them. Nothing is hidden behind a click — the previews ARE the pitch, and a
  // stranger won't click to see your pitch. The count on each chip answers "what do I get
  // for nothing?" at a glance. Desktop only: #flt lives in the landing hero (not in the
  // /subscribe slice), and its band-free / band-prem targets only exist when the two flat
  // tiers render — on a phone the four group headings do the navigating, and the pill is
  // display:none. Guarded so the /subscribe embed (no #flt) doesn't throw.
  if(flt){
  ...
  // Keep the lit chip matching whatever band is in view, so the control reports position
  // instead of just firing and forgetting. No-ops when the bands aren't on the page.
  function spy(){
    if(!flt) return;
  ...
  // DESKTOP collapses the four named groups to two flat lists — Free and Premium. The
  // chunking exists so a phone isn't one long 15-card scroll; on a wide screen it does the
  // opposite of helping — a 3-card group that happens to hold the full-width hub card leaves
  // its other two cards marooned in half-empty rows. A single Premium grid packs the hub in
  // among eleven cards instead. The four named SWIPE groups stay on mobile.
  var TIERS2=[
    {k:'free', label:'Free',    note:'play, predict and dig in — free forever', tier:'free', sel:function(s){ return s.f; }},
    {k:'prem', label:'Premium', note:'the whole database and every tool',        tier:'prem', sel:function(s){ return !s.f; }}
  ];
  function isWideLayout(){ return window.matchMedia('(min-width:721px)').matches; }

  function paint(){
    tiers.innerHTML='';
    // Desktop: Free / Premium (flat). Mobile: the four named groups, matched by slide.g.
    var defs = isWideLayout()
      ? TIERS2.slice()
      : GROUPS.map(function(g){ return {k:g.k,label:g.label,note:g.note,tier:g.tier,sel:function(s){ return s.g===g.k; }}; });
    // /subscribe embeds this with window.__FX_PREMIUM_ONLY=true — drop the free tier there
    // (on desktop that leaves one Premium list; on mobile the three premium swipe groups).
  ...
  // the layout (two flat lists <-> four swipe groups) instead of freezing at load width.
  var _wasWide = isWideLayout();
  window.addEventListener('resize', function(){
    var w = isWideLayout();
    if(w!==_wasWide){ _wasWide=w; paint(); }
  });
