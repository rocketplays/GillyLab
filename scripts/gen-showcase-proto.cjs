...
${slideJS}
  ...
  function paint(){
    tiers.innerHTML='';
    // /subscribe embeds this same grid with window.__FX_PREMIUM_ONLY=true and renders only
    // the premium groups — the free tier is not a selling point on the checkout page. On
    // the landing (flag unset) all four groups render as before.
    var groups = window.__FX_PREMIUM_ONLY ? GROUPS.filter(function(g){ return g.tier==='prem'; }) : GROUPS;
    ...
    // Every slide must land in a group, or it silently vanishes from the page. (Skip the
    // check in premium-only mode, where the free slides are deliberately not placed.)
    if(!window.__FX_PREMIUM_ONLY){
