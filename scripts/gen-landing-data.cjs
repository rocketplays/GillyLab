...
function main() {
  ...
  let mains = {};
  if (card && card.main) mains[card.slug] = card.main;
  try {
    ...
    console.log('landing-data.js: main-event breakdown for ' + Object.keys(mains).length + ' event(s) (' + carouselRaws.length + ' carousel candidates checked)');
  } catch (e) {
    console.error('gen-landing-data: could not build carousel main-event breakdowns (event.json):', e.message);
  }
  const out = { generatedAt: new Date().toISOString(), rankings, roster, featured, oddsHistory, matchup, parlay, styleDemo, counts, photos, card, held, mains };
