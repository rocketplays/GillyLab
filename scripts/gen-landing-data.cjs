...
function buildCard(recMap, evOverride) {
  ...
  // Same style/pace/path/storylines/finish-rate/common-opponents breakdown
  // as `main` above, but for EVERY bout on the card, not just the headliner --
  // premium users get the full Fight Info dropdown on every fight (mirroring
  // the site, which computes this live for every bout via renderScouting()),
  // not just the main event. fights[0] reuses `main` itself rather than
  // calling buildMainTape() a second time for the same pair.
  fights.forEach((f, i) => { f.breakdown = i === 0 ? main : buildMainTape(f); });
...
function main() {
  ...
  // Main-event breakdown (style/pace/path/storylines/tale-of-the-tape) for
  // /matchup's own upcoming-events carousel, not just the featured card. Was
  // only ever built for `card` above -- every carousel slide's own main event
  // fell back to the locked-teaser panel purely because this map didn't exist
  // yet, not because the analysis is expensive: buildMainTape() is a pure
  // function of two names over data already loaded into memory, so reusing
  // buildCard(recMap, raw) per upcoming event (same trick `held` above already
  // uses for the one finished card) is cheap. Same event-selection filters as
  // the Worker's own carousel (worker/index.js's carouselRaws, worker/pages.js's
  // matchupPage) so the set of events that get a breakdown here is exactly the
  // set that will ever ask for one — kept in three places because the Worker
  // needs the filter at request time (it doesn't know the build-time card set)
  // and this script has no import path into worker/pages.js's own copy.
  const hasBoutsRaw = (e) => !!(e && e.bouts && e.bouts.length);
  const isRoadToUFCRawLocal = (e) => /road\s+to\s+(the\s+)?ufc/i.test((e && (e.espnName || e.title || e.shortTitle || e.name)) || '');
  let mains = {};
  // Per-FIGHT breakdown (not just the main event) for every card this build
  // touches, keyed by event slug -- an array of {f1,f2,breakdown} aligned to
  // that event's own bout order. `card`/`held.card` already carry this
  // directly on each fight (buildCard() attaches it -- see its own comment),
  // so the app's featured/held view needs nothing further; this map exists
  // for the carousel/`?event=` paths, which build their fights through
  // worker/pages.js's eventToCard() instead (a different function, in the
  // Worker runtime, with no access to the stats/history data this script
  // reads from index.html) -- worker/index.js matches each of THOSE fights
  // back to an entry here by fighter name to attach the same breakdown.
  let fightBreakdowns = {};
  const fbEntries = (built) => (built && built.fights || []).map((f) => ({ f1: f.f1, f2: f.f2, breakdown: f.breakdown || null }));
  if (card) { if (card.main) mains[card.slug] = card.main; fightBreakdowns[card.slug] = fbEntries(card); }
  if (held && held.card) fightBreakdowns[held.card.slug] = fbEntries(held.card);
  try {
    const feed = JSON.parse(fs.readFileSync(path.join(ROOT, 'data', 'event.json'), 'utf8'));
    const featuredTime = card && (card.prelimsAt || card.date) ? Date.parse(card.prelimsAt || card.date) : NaN;
    const carouselRaws = ((feed && feed.data) || [])
      .filter((e) => e && e.status !== 'completed')
      .filter(hasBoutsRaw)
      .filter((e) => !isRoadToUFCRawLocal(e))
      .filter((e) => !card || e.slug !== card.slug)
      .filter((e) => !card || !isFinite(featuredTime) || Date.parse(e.startsAt || 0) > featuredTime)
      .slice(0, 12);
    for (const raw of carouselRaws) {
      try {
        const built = buildCard(recMap, raw);
        if (built && built.main) mains[built.slug] = built.main;
        if (built) fightBreakdowns[built.slug] = fbEntries(built);
      } catch (e) { console.error('gen-landing-data: main breakdown failed for ' + (raw && raw.slug) + ':', e.message); }
    }
    console.log('landing-data.js: main-event breakdown for ' + Object.keys(mains).length + ' event(s), per-fight breakdown for ' + Object.keys(fightBreakdowns).length + ' event(s) (' + carouselRaws.length + ' carousel candidates checked)');
  } catch (e) {
    console.error('gen-landing-data: could not build carousel main-event breakdowns (event.json):', e.message);
  }
  const out = { generatedAt: new Date().toISOString(), rankings, roster, featured, oddsHistory, matchup, parlay, styleDemo, counts, photos, card, held, mains, fightBreakdowns };
