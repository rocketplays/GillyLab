...
def parallel_fighter(sid, namecache, bout_workers):
    ...
    try:
        log = fsb.getj("%s/%s/eventlog" % (fsb.ATHL, sid))
    except Exception:
        return None
    pairs = []
...
def main():
    ...
    print("roster: %d | already saved: %d | to process: %d" % (len(names), len(data), len(todo)))
    if not todo: return
    ...
    def work(nm):
        sid = get_id(nm)                       # bulk: only fighters with a known id (search stragglers later)
