...
// DATA/CODE SPLIT (Sep 2026 cold-start latency fix -- see git log and
// CLAUDE.md #1/#3's own eager-payload warning). The full `out` object used
// to be baked straight into worker/fighter-extras.js as a 19MB
// `export default {...}` literal. Because wrangler.toml's run_worker_first
// means the Worker script runs on EVERY request (even a cached static
// asset), V8 had to parse and compile that literal on every cold isolate --
// not just requests that actually needed it. Now: the data goes to
// data/fighter-extras-full.json (built and deployed like any other
// data/*.json -- NOT excluded from build-site.sh; it stays protected the
// same way every other /data/*.json the Worker serves via env.ASSETS
// already is, see that file's own header comment), and worker/
// fighter-extras.js is a tiny loader that fetches + caches it once per
// isolate, lazily, only when a request actually calls getFighterExtras(env).
const DATA_OUT = R('data/fighter-extras-full.json');
...
console.log('data/fighter-extras-full.json  ' + kb(json.length) + '\n' +
  'worker/fighter-extras.js  ' + kb(mod.length) + ' (loader only)' +
