...
// ── render ───────────────────────────────────────────────────────────────────
let out = { slug: ev.slug, n1, n2, striking: '', grappling: '', css: hubCSS, generatedAt: new Date().toISOString() };
...
if (!ctx.glDeepDiveAvailable(n1, n2)) {
  ...
  const A = ctx._ddGrid(n1), B = ctx._ddGrid(n2);
  if (!A || !B) throw new Error('glDeepDiveAvailable said yes but _ddGrid returned null for ' + n1 + ' vs ' + n2);
  out.striking = ctx.mhStriking(A, B, n1, n2);
  out.grappling = ctx.mhGrappling(A, B, n1, n2);
  // The panel must never ship a NaN or an [object Object] to a public page.
  for (const [k, v] of [['striking', out.striking], ['grappling', out.grappling]]) {
    if (!v || v.length < 200) throw new Error(k + ' rendered empty/short — refusing to ship it');
...
console.log('worker/matchup-free.js  ' + kb(mod.length) +
  '  (' + n1 + ' vs ' + n2 + ', ' + ev.slug + ')' +
  '  striking ' + kb(out.striking.length) + ' · grappling ' + kb(out.grappling.length) + ' · css ' + kb(out.css.length) +
